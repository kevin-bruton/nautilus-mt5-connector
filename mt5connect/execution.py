...
class MT5LiveExecutionClient(LiveExecutionClient):
    ...
    def __init__(
        ...
        # Strategy ownership maps used to preserve Nautilus attribution on fills
    ...
    async def _emit_fill(self, deal) -> None:
        ...
        The ClientOrderId and StrategyId are recovered from the ticket maps
        populated when this client submitted the order. Fills without recoverable
        ownership are skipped because Nautilus cannot apply them to a cached
        order without corrupting strategy attribution.
        """
        ...
        # Recover ClientOrderId and StrategyId for orders submitted this session.
        ...
        if strategy_id_str is None:
            self._log.debug(
                f"MT5LiveExecutionClient: deal {deal.ticket} (order={deal.order}) "
                f"client_order_id={client_order_id_str} has no strategy_id — "
                "skipping fill emission"
            )
            return
